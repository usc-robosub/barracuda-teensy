...
# set PWM_FREQ, PWM_BIT_RES, T200_INIT_PWM_US
...
NTHRUSTERS = 4
ARDUINO_PWM_PINS = [3, 5, 6, 9] 
PWM_FREQ = 500
PWM_BIT_RES = 8
INIT_DC = round((T200_INIT_PW * (PWM_FREQ / 1e6)) * 2**PWM_BIT_RES)
...
dc_to_us = lambda dc: dc / (2**PWM_BIT_RES) / (PWM_FREQ / 1e6)

dc_vals = [dc for dc in range(2**PWM_BIT_RES) if T200_MIN_PW <= dc_to_us(dc) <= T200_MAX_PW]
pw_vals = [dc_to_us(dc) for dc in dc_vals]
f_vals = [pw_to_f(pw) for pw in pw_vals]

for dc, pw, f in zip(dc_vals, pw_vals, f_vals):
    print(dc, pw, f)


out = f'''\
constexpr int NTHRUSTERS = {NTHRUSTERS};
constexpr int PWM_PINS[] = {{ {','.join(map(str, ARDUINO_PWM_PINS))} }};
constexpr float PWM_FREQ = {PWM_FREQ};
constexpr int PWM_BIT_RES = {PWM_BIT_RES};
constexpr uint16_t INIT_DC = {INIT_DC};

constexpr int LUT_LEN = {len(dc_vals)};
constexpr uint16_t dc_vals[] = {{ {','.join(map(str, dc_vals))} }};
constexpr float f_vals[] = {{ {','.join([f'{val:.2f}f' for val in f_vals])} }};
'''

with open('barracuda-arduino.h', 'w') as f:
    f.write(out)